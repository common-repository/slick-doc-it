...
<a class="buy-extensions-btn" href="http://www.slickremix.com/downloads/doc-it-premium-extension/" target="_blank">Get Extension Here!</a> 
...
<?php
   ...
   <form method="post" id="doc-it-settings-taxes" name="createMenu" action="options.php">
    ...
    <iframe id="slickdocit-iframe" src="https://www.youtube.com/embed/h6JLEDmsNWk" data-autoplay-src="https://www.youtube.com/embed/h6JLEDmsNWk?autoplay=1" frameborder="0" allowscriptaccess="always" allowfullscreen> </iframe>
</div></div>
	                <div class="use-of-plugin">
                 				<div id="slickdocit-show-video" class="slickdocit-show-video">View Quick Setup Video<span class="slickdocit-play"></span> </div>
                     <br/>
	                    <p>1. Input the Main Name for what you are going to document and save it. (ie. My Project, My Book, My Plugin, etc...) This creates an admin menu item. (on the left of this page under "DocIt" tab) This admin menu item is where you will go to make the categories for the item you are documenting.</p> Maximum of 32 characters allowed for names.
	       
	                </div>
<script>
			jQuery( ".slickdocit-show-video" ).click(function() {
					var vi = jQuery("#slickdocit-iframe");
    vi.attr("src", vi.data("autoplay-src") );
				
 			 jQuery( ".slickdocit-videowrapper" ).slideDown();
					jQuery('.slickdocit-show-video').fadeOut();
});	
</script>
	       	<?php  
				if (is_plugin_active('doc-it-premium/doc-it-premium.php')) {
					...
					$final_count = $doc_it_array == '1' ? 1 : $doc_it_array + 1;
								  ...
								  echo '<input name="doc_it_menu_labelz['.$final_count.']" id="doc-it-create-id" placeholder="Letters and numbers only" maxlength="29" class="doc-it-settings-admin-input doc-it-custom-name" type="text"  />';
				      echo'<br/>';
	        ...
	        <?php if (!empty($options)) {
				...
				echo'<br/>';
					...
					echo'<input name="doc_it_menu_labelz[1]" id="doc-it-create-id" placeholder="Letters and numbers only" maxlength="29" class="doc-it-settings-admin-input doc-it-custom-name" type="text"/>';
					echo'<br/>';
				}
				if (!empty($options)) {
					foreach ($options as $key => $value) {
...
<?php  if(is_plugin_active('doc-it-premium/doc-it-premium.php')) { ?> 
 <div class="doc-it-settings-admin-input-wrap company-info-style" id="di-premium-content"> 
         <div class="use-of-plugin-2"><h1>Premium Content</h1> 
    This area is to allow a limited view of content while users are logged out.
     </div>  
     
     <div class="doc-it-color-settings-admin-input-label docit-wp-header-custom">Check the box below to control the premium content options. Choose the amount of words you want to display when users are logged out below and also the text you want for the button that will appear for users to login under the faded out text. Leave Button Text blank for no login button.</div>
   
   <p>
      <input name="doc-it-premium-content" class="doc-it-color-settings-admin-input" type="checkbox"  id="doc-it-premium-content" value="1" <?php echo checked( '1', get_option( 'doc-it-premium-content' ) ); ?>/>
        		   <?php  
                        if (get_option( 'doc-it-premium-content' ) == '1') {
                           echo "<strong>Checked:</strong> You are NOW using Premium Content.";
                        }
                        else	{
                          echo "<strong>Not Checked:</strong> You are NOT using Premium Content.";
                        }
                     ?>
         </p>
               
         <p>
         <label>Post Words:</label> <input name="doc-it-logged-out-word-count" class="doc-it-color-settings-admin-input" type="text"  id="doc-it-logged-out-word-count" placeholder="250 for example" value="<?php echo get_option('doc-it-logged-out-word-count'); ?>" title=""/>
      				</p>
          
           <p>
         <label>List Words: </label> <input name="doc-it-list-words" class="doc-it-color-settings-admin-input" type="text"  id="doc-it-list-words" placeholder="80 for example" value="<?php echo get_option('doc-it-list-words'); ?>" title=""/>
      				</p>
      
      <p><label>Button Text: </label> <textarea name="doc-it-logged-out-button-text" class="doc-it-color-settings-admin-input" id="doc-it-logged-out-button-text" style="width:50%; min-height:150px;"><?php echo get_option('doc-it-logged-out-button-text'); ?></textarea></p>
    
    
     <div class="doc-it-color-settings-admin-input-label docit-wp-header-custom">The checkbox below will add a small menu above your Doc It sidbar and it will have a home icon, and the word Home and Logout. The Menu Link should lead you to the page where you pasted your shortcode which would be your Home page for Doc It.</div>
       <p>
      <input name="doc-it-add-menu-above-sidebar" class="doc-it-color-settings-admin-input" type="checkbox"  id="doc-it-add-menu-above-sidebar" value="1" <?php echo checked( '1', get_option( 'doc-it-add-menu-above-sidebar' ) ); ?>/>
        		   <?php  
                        if (get_option( 'doc-it-add-menu-above-sidebar' ) == '1') {
                           echo "<strong>Checked:</strong> You are using the menu above the sidebar.";
                        }
                        else	{
                          echo "<strong>Not Checked:</strong> You are NOT using the menu above the sidebar.";
                        }
                     ?>
         </p>
          <p>
         <label>Menu Title:</label> <input name="doc-it-custom-menu-name" class="doc-it-color-settings-admin-input" type="text"  id="doc-it-custom-menu-name" placeholder="eg Home" value="<?php echo get_option('doc-it-custom-menu-name'); ?>" title=""/>
      				</p>
          
           <p>
         <label>Menu Link: </label> <input name="doc-it-custom-menu-link" class="doc-it-color-settings-admin-input" type="text"  id="doc-it-custom-menu-link" placeholder="" value="<?php echo get_option('doc-it-custom-menu-link'); ?>" title=""/>
      				</p>
          <br/>
  ...
  <!-- custom option for padding -->
    ...
    <p>
     ...
     <p>
      ...
      <br/>
       <p>
        <input name="doc-it-hide-breadcrumbs" class="doc-it-color-settings-admin-input" type="checkbox"  id="doc-it-hide-breadcrumbs" value="1" <?php echo checked( '1', get_option( 'doc-it-hide-breadcrumbs' ) ); ?>/>
        <?php  
                        if (get_option( 'doc-it-hide-breadcrumbs' ) == '1') {
                           echo "<strong>Checked:</strong> You are hiding the Breadcrumbs."; 
                        }
                        else	{
                          echo "<strong>Not Checked:</strong> The Breadcrumbs are visible.";
                        }
                           ?>
       </p>
      <br/>
       <p>
        <input name="doc-it-hide-footer-navigation" class="doc-it-color-settings-admin-input" type="checkbox"  id="doc-it-hide-footer-navigation" value="1" <?php echo checked( '1', get_option( 'doc-it-hide-footer-navigation' ) ); ?>/>
        <?php  
                        if (get_option( 'doc-it-hide-footer-navigation' ) == '1') {
                           echo "<strong>Checked:</strong> You are hiding the Footer Navigation on the Doc It Posts.";
                        }
                        else	{
                          echo "<strong>Not Checked:</strong> You are using the Footer Navigation on the Doc It Posts.";
                        }
                           ?>
       </p>
      <br/>
