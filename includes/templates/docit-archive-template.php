    ...
    <div id="doc-it-content-wrap">
     <article id="docit-post-<?php the_ID(); ?>" <?php post_class(); ?>>
        <header class="docit-entry-header">
   					 <h1 class="header-text-docit"><?php single_cat_title(''); ?></h1>
        </header>
        <!-- .docit-entry-header -->
      <?php 
	  global $query_string;
      ...
      <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
        <header class="doc-it-entry-header">
          ...
          <div class="doc-it-entry-meta">
            <?php edit_post_link('<span class="docit-icon-edit"></span>', '<span class="edit-link">', '</span>' ); ?>
          </div>
          <!-- .doc-it-entry-meta --> 
        </header>
        <!-- .doc-it-entry-header -->
		...
		<?php 	if(is_plugin_active('doc-it-premium/doc-it-premium.php')) {
             if (get_option( 'doc-it-premium-content' ) == '1' && get_option( 'doc-it-list-words' ) !== '') {
                  echo $DI_core->slick_docit_cat_list_content(get_option( 'doc-it-list-words' ));
               }
